...
$results = $subscriptions | ForEach-Object -ThrottleLimit 10 -Parallel {
    ...
    # Walk through each VNet and enumerate its subnets with device counts.
    $subnetDetails = foreach ($vnet in $vnets) {
...
$allSubnetDetails = $results.SubnetDetails
...
$tableRows += $scopedSubnets | ForEach-Object {
    [PSCustomObject]@{
        Status       = if ($_.DeviceCount -eq 0) { "Empty" } else { "In Use" }
...
$tableRows += $missingSubnets | ForEach-Object {
    [PSCustomObject]@{
        Status       = "Not Found"
        Subscription = "-"
        VNet         = "-"
...
$sortedRows = @($tableRows | Sort-Object Status, Subscription, VNet, Subnet)
